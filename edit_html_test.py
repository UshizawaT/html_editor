...
def get_indexlist_on_partial_match(searchlist: list[str], reference: str, n_index:int=0) -> int:
    l_out:list[int] = [i for i,l in enumerate(searchlist) if reference in l and i>n_index]
    return l_out
...
def is_tag_inside(rawlines: list[str], outer_tag: list[str], candidate_tag: str, target_from_ctag:int, start_index:int=0)->int:
    i_otag_st, i_otag_en = get_index_of_outer_tags(rawlines, outer_tag, start_index)
    ...
    if i_otag_st <= _i_mod <= i_otag_en:
        ...
        while True:
            i_otag_st2, i_otag_en2 = get_index_of_outer_tags(rawlines, outer_tag, i_otag_en)
...
def file_io(filepath: str, f_insert:bool, f_replace:bool):
    ...
    if f_insert:
        if d_json['insert_flag'] == 'upper':
            print('insert upper')
            l_wtxt = insert_before_candidate_tag(l_txt, o_tag, c_tag, d_json['insert_tag'])
        elif d_json['insert_flag'] == 'bottom':
            print('insert bottom')
            l_wtxt = insert_after_candidate_tag(l_txt, o_tag, c_tag, d_json['insert_tag'])
        f_sucsess = (len(l_wtxt)>0)
    elif f_replace:
        print('start replacement!')
        l_wtxt, f_sucsess = replace_tag(l_txt, o_tag, c_tag, d_json['replace_tag'])
    if not f_sucsess:
        return False
